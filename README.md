...
- Особенности:
  ...
  - Валидация: 
    - цена > 0 
    - количество > 0 (вызывает ValueError при нулевом количестве)
...
- Методы:
  ...
  - `middle_price()` — вычисляет среднюю цену товаров в категории (возвращает 0 для пустой категории)
  - `__str__` — строковое представление в формате "Название, количество продуктов: X шт."

## Тестирование
- Полное покрытие тестами всех классов (>90%)
- Новые тесты:
  - Проверка создания продукта с нулевым количеством (ValueError)
  - Проверка метода `middle_price()` для:
